...
<template>
  <div class="container-fluid px-4 py-6 overflow-x-hidden min-h-screen">
    ...
    <div class="text-center mb-8" :class="{ 'animate-fade-in': isLoaded }">
      <h1 class="text-4xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-primary via-secondary to-accent">探索课程</h1>
      <p class="text-base-content/70">发现精心设计的安全课程，开启你的学习之旅</p>
    </div>

    <!-- 统计卡片 -->
    <div class="stats shadow w-full mb-8 bg-base-100 backdrop-blur-sm" :class="{ 'animate-slide-up': isLoaded }">
      <div class="stat">
    ...
    <!-- 搜索和筛选 -->
    <div class="flex flex-col md:flex-row gap-4 mb-8" :class="{ 'animate-slide-up': isLoaded }" style="animation-delay: 0.2s">
      <!-- 搜索框 -->
      <div class="flex-1 relative">
        <input type="text" placeholder="搜索课程..." class="input input-bordered w-full pl-10 pr-4 bg-base-100/70 focus:bg-base-100 transition-all duration-300 shadow-sm hover:shadow focus:shadow-md" v-model="searchQuery" />
        <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-base-content/50"></i>
      </div>

      <!-- 分类选择 -->
      <div class="flex gap-2">
        <button v-for="category in categories" :key="category.id" class="btn btn-ghost gap-2" :class="{ 'btn-active': selectedCategory === category.id }" @click="selectedCategory = selectedCategory === category.id ? '' : category.id">
    ...
    <!-- 课程列表 -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" :class="{ 'animate-fade-in': isLoaded }" style="animation-delay: 0.3s">
      <template v-if="filteredCourses.length">
        <div v-for="(course, index) in filteredCourses" :key="course.id" :style="{ animationDelay: `${index * 0.1}s` }" class="animate-slide-up">
      ...
      <div v-else class="col-span-full text-center py-12">
        ...
        <p class="text-base-content/70">试试调整搜索条件</p>
      </div>
    </div>
  </div>
</template>
